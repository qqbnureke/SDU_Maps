                ...
                if (location != null || !location.equals("")) {
                    ...
                    try {
                        for (int i = 0; i < markers.size(); i++) {
                            String mm = markers.get(i).getTitle().toLowerCase();
                            if (mm.matches("(.*)" + query.toLowerCase() + "(.*)")) {
                                latLng = markers.get(i).getPosition();
                            }
                        }
...
//                    latLng = new LatLng(address.getLatitude(), address.getLongitude());
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        GroundOverlayOptions newarkMap = new GroundOverlayOptions()
                .image(BitmapDescriptorFactory.fromResource(R.drawable.sdu))
        ...
        //add markers
        /*
        RedHall = mMap.addMarker(new MarkerOptions()
                .position(new LatLng(43.208761, 76.670166))
                .title("Red Hall")
                .icon(BitmapDescriptorFactory.fromResource(R.drawable.hall))
                .zIndex(1.0f));

        Cattery = mMap.addMarker(new MarkerOptions()
                .position(new LatLng(43.207437, 76.669583))
                .title("Dining Room")
                .icon(BitmapDescriptorFactory.fromResource(R.drawable.dining))
                .zIndex(1.0f));

        B1 = mMap.addMarker(new MarkerOptions()
                .position(new LatLng(43.208130, 76.669516))
                .title("B1 lecture")
                .icon(BitmapDescriptorFactory.fromResource(R.drawable.lecture))
                .zIndex(1.0f));

        markers.addAll(Arrays.asList(B1, Cattery, RedHall));

        mMap.addMarker(new MarkerOptions().position(new LatLng(43.207436, 76.670144)).title("Library"));
        */
        drawMarkers(-1);
